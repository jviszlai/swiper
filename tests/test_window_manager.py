...
merge_schedule.idle([(0,100)], 10)
merge_schedule.merge([(0,0), (0,100)], [(0,i) for i in range(1,100)])
merge_schedule.discard([(0,0), (0,100)])
...
def test_sliding_idle():
   ...
   while not simulator.is_done():
      ...
      # TODO: check windows as they come out
...
def test_parallel_regularT():
   """Test that ParallelWindowManager can correctly handle a regular T schedule."""
   simulator = DecodingSimulator(7, lambda _: 14, 2, 0, speculation_mode='separate')
   ...
   device_data, window_data, decoding_data = simulator.get_data()
   # assert set(window_data.window_dag.nodes) ==
   # set(range(len(window_data.all_windows)))
   ...
   #                               raise ValueError("Failed to resolve sink-sink or source-source conflict")

if __name__ == '__main__':
   test_parallel_regularT()