...
if __name__ == '__main__':
    ...
    for file in os.listdir('benchmarks/cached_schedules/'):
        ...
        if file.endswith('.lss') and file.startswith('rz_seq_10.lss'):
    ...
    decoder_latencies = [f'lambda volume: volume*{fac}' for fac in np.geomspace(0.1, 10, 10)]
    sweep_params = {
        'benchmark_file':benchmark_files,
        'distance':[21],
        'scheduling_method':['sliding', 'parallel', 'aligned'],
        'decoder_latency_or_dist_filename':decoder_latencies,
    ...
    def config_filter(cfg):
        return (cfg['rng'] == decoder_latencies.index(cfg['decoder_latency_or_dist_filename']))
