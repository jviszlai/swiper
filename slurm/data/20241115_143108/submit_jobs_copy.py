...
if __name__ == '__main__':
    ...
    for file in os.listdir('benchmarks/cached_schedules/'):
        ...
        if file.endswith('.lss') and not file.startswith('memory') and not file.startswith('regular') and not file.startswith('random') and float(benchmark_info[file.split('.')[0]]['Ideal volume']) < 80_000:
    ...
    def config_filter(cfg):
        return (not (cfg['scheduling_method'] == 'sliding' and cfg['speculation_mode'] == None)) and (cfg['rng'] == 0 or (cfg['rng'] > 0 and cfg['benchmark_file'].split('/')[-1] in microbenchmarks))
